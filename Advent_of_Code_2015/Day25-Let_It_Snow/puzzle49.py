...
# The weather machine beeps! On the console of the machine is a copy protection message asking you to enter a code from the 
# instruction manual. Apparently, it refuses to run unless you give it that code. No problem; you'll just look up the code 
# in the--
...
# The codes are printed on an infinite sheet of paper, starting in the top-left corner. The codes are filled in 
# by diagonals: starting with the first row with an empty first box, the codes are filled in diagonally up and to the right. 
...
# one at 7,1 that would come before 6,2. But, it should be enough to let your-- oh, it's time for lunch! Bye!" 
...
#We have value for (1,1) so we start calculating from next one which is on (2,1)
startRow = 2
startColumn = 1

#Our goal is (2947,3029)
goalRow = 2947
goalColumn = 3029
#Number on (1,1)
firstNumber = 20151125
#Multiplicator given in puzzle
multiplicator = 252533
#Devider given in puzzle
devider = 33554393

#Set our number to the first number
number = firstNumber
#Set flag which controls when we need to exit loop to false
found = False

#Loop over each diagonal unitl we find solution
while True:
    ...
    #While we don't reach end of diagonal
    while row >= 1:

        #Calculate the number with the given formula
        number = (number*multiplicator) % devider

        #If we reached our goal position, exit loops
    ...
    #If solution is found, break loop   
    if found:
        break
...
print(number,"is the code you give the machine.")