...
#Dictionary of posititons
positions = dict()

#For each report
for line in diagnosticReport:

    #For each bit in report
    for index in range(len(line)):

        #If the current bit index is not in dictionary, add it
        if index not in positions:
            positions[index] = [0,0]

        #If the current bit is 0, increase zero coutner, else increase one counter
        if line[index] == "0":
            positions[index][0] += 1
        else:
            positions[index][1] += 1
...
for key in sorted(positions.keys()):
    ...
    #Append its more common value to gamma, and less common value to epsilon
    if positions[key][0] > positions[key][1]:
        gamma += "0"
        epsilon += "1"
    else:
        gamma += "1"
...
#Output
print("The power consumption of the submarine:",gamma*other)