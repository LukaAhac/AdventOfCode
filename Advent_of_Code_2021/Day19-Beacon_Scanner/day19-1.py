...
# the scanner could report that the first beacon is at -1000,1000,-1000 (relative to the scanner) but would not 
...
# B....S
# ....B.
# For this example, assume scanners only need 3 overlapping beacons. Then, the beacons visible to both scanners 
# overlap to produce the following complete map:
...
# Or, two scanners might agree on which direction is positive x, but one scanner might be upside-down from 
# the perspective of the other scanner. In total, each scanner could be in any of 24 different orientations: 
...
# -652,-548,-490
# 30,-46,-14
# Because all coordinates are relative, in this example, all "absolute" positions will be expressed relative to 
# scanner 0 (using the orientation of scanner 0 and as if scanner 0 is at coordinates 0,0,0).

# Scanners 0 and 1 have overlapping detection cubes; the 12 beacons they both detect (relative to scanner 0) 
...
# Following this process, scanner 2 must be at 1105,-1205,1229 (relative to scanner 0) and scanner 3 must 
# be at -92,-2380,-20 (relative to scanner 0).
...
#Put all the points (but relative to the (0,0,0)) from all scanners into the set so we can get the number of
#unique ones
uniqPoints = set()

for scanner in scannerReadingsCopy.keys():

    x = solvedScanners[scanner][3]
    y = solvedScanners[scanner][4]

    for p in scannerReadingsCopy[scanner]:

        #Rotate the point and add the scanner distance so we get point distance to the (0,0,0)
        r2 = rotatePoint(p,x,y)

        r2[0] += solvedScanners[scanner][0]
        r2[1] += solvedScanners[scanner][1]
        r2[2] += solvedScanners[scanner][2]

        newPoint = (r2[0],r2[1],r2[2])
        uniqPoints.add(newPoint)

print("There are {} beacons.".format(len(uniqPoints)))