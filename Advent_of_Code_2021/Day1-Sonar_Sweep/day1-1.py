...
increased = 0
#Variable that holds last reading
last = None

#For each mesurement check if it is bigger than the previous one
for current in mesurements:

    #If there is not last, then just make the last = current
    if last is None:
        last = current
        continue

    #Else if current is bigger than last increase the counter
    elif current > last:
        increased += 1
...
#Output
print(increased,"measurements are larger than the previous measurement!")