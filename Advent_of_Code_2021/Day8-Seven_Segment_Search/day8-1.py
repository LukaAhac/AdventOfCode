...
# ou barely reach the safety of the cave when the whale smashes into the cave mouth, collapsing it. Sensors indicate 
...
# displays in your submarine are malfunctioning; they must have been damaged during the escape. You'll be in a lot 
...
# means that to render a 7, signal lines d, a, and b are on. Because 4 is the only digit that uses four segments, 
...
# the ten digits. Then, you can decode the four digit output value. Unfortunately, in the above example, all of the 
...
for line in inp:
    ...
    #Look only last 4 numbers (which are output values)
    for digit in line[-4:]:

        #If it is the lenght of 2,3, 4 or 7, then count it
