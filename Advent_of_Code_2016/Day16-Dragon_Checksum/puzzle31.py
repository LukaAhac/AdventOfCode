...
# You need to overwrite some disks with random-looking data to cover your tracks and update the 
# local security system with a new checksum for those disks.

# For the data to not be suspicious, it needs to have certain properties; purely random data will 
# be detected as tampering. To generate appropriate random data, you'll need to use a modified dragon curve.
...
# Calculate the checksum only for the data that fits on the disk, even if you generated more 
# data than that in the previous step.

# The checksum for some given data is created by considering each non-overlapping pair of 
# characters in the input data. If the two characters match (00 or 11), the next checksum character is a 1. 
...
# string of at least length 12, the first 12 characters are 110010110100. To generate its checksum:
